...
function displacement_error(
    ...
    exactnorm = integral_norm_on_interface(
        analyticalsolution,
        interfacequads,
        +1,
        mergedmesh,
...
@test all(productu1rate .> 2.8)
@test all(productu2rate .> 2.8)
@test all(parentu1rate .> 2.8)
@test all(parentu2rate .> 2.8)
